...
def search_skills(con: duckdb.DuckDBPyConnection, query: str, limit: int = 20) -> list[dict]:
    ...
    Đối sánh chạy trên `skill_terms` (đã bỏ dấu và có dạng dính liền) nên "tieng
    ...
    sql = """
        SELECT s.skill_id, s.canonical_name, s.skill_type, s.category,
               max(CASE
                   ...
                   END) AS match_rank
        FROM skills s
        JOIN skill_terms t ON t.skill_id = s.skill_id
...
def get_skill_detail(con: duckdb.DuckDBPyConnection, skill_id: str) -> dict | None:
    base = con.execute("SELECT * FROM skills WHERE skill_id = ?", [skill_id]).fetchdf()
    if base.empty:
        return None
    ...
    if info.get("parent_skill_id"):
        prow = con.execute(
            "SELECT skill_id, canonical_name FROM skills WHERE skill_id = ?",
    ...
    children = con.execute(
        "SELECT skill_id, canonical_name FROM skills WHERE parent_skill_id = ? ORDER BY canonical_name",
        [skill_id],
    ).fetchdf().to_dict("records")

    variants = con.execute(
        "SELECT surface_form FROM skill_variants WHERE skill_id = ? ORDER BY surface_form",
        [skill_id],
    ).fetchdf()["surface_form"].tolist()

    job_count = con.execute(
        "SELECT count(DISTINCT job_id) FROM job_skills WHERE skill_id = ?", [skill_id]
    ).fetchone()[0]

    evidence = con.execute(
        """
        SELECT source, extraction_method, confidence, evidence_snippet
        FROM job_skills WHERE skill_id = ?
...
def expand_skill_ids(con: duckdb.DuckDBPyConnection, skill_id: str) -> list[str]:
    rows = con.execute(
        "SELECT descendant_id FROM skill_closure WHERE ancestor_id = ?", [skill_id]
    ).fetchdf()
...
def search_jobs(
    ...
    sql = f"""
        ...
        JOIN skills sk ON sk.skill_id = f.skill_id
...
def _job_scope(city: str | None = None, country: str | None = None) -> tuple[list[str], list]:
    """Điều kiện lọc ở mức tin, dùng chung cho mọi truy vấn thống kê.

    Các truy vấn dưới đều `LEFT JOIN locations l` để dùng được điều kiện này mà
...
def top_skills(
    ...
    clauses, params = _job_scope(city, country)
    if skill_type:
...
def skill_by_city(
    ...
    """
    clauses, params = _job_scope(None, country)
...
def hard_soft_ratio(con: duckdb.DuckDBPyConnection) -> dict[str, int]:
    """Số tin có yêu cầu kỹ năng cứng / kỹ năng mềm.

    Đếm theo tin chứ không theo số cặp để cùng đơn vị với `top_skills`; một tin đòi
